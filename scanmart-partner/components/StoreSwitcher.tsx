...
export default function StoreSwitcher() {
  ...
  const checkUserRoleAndStores = async () => {
    ...
    if (!user) {
      // Not logged in via Supabase auth — use localStorage store (staff mode)
    ...
    // Check if this auth user owns any stores
    ...
    const isOwner = ownerStores && ownerStores.length > 0;
    setIsAdmin(!!isOwner);
