      ...
      <div className="min-h-screen bg-[#020617] flex items-center justify-center text-white p-4">
        <div className="bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-md border border-blue-500/30 shadow-2xl text-center relative overflow-hidden">
          ...
          <div className="space-y-4 text-left">
            ...
            <div><label className="text-[10px] font-bold uppercase text-slate-500 ml-2">Create Admin PIN <span className="text-yellow-400">(6 digits)</span></label><input type="password" inputMode="numeric" maxLength={6} placeholder="XXXXXX" className="w-full bg-slate-950 border border-slate-800 p-4 rounded-2xl text-white font-bold text-center tracking-[1rem] outline-none focus:border-blue-500 transition-all" value={ownerDetails.pin_code} onChange={(e) => setOwnerDetails({ ...ownerDetails, pin_code: e.target.value.replace(/\D/g, '') })} /></div>
          </div>
    ...
    return (
      <div className="fixed inset-0 z-[9999] bg-[#020617] flex items-center justify-center text-white p-4 backdrop-blur-sm">
        <div className="bg-slate-900 p-8 rounded-[2rem] w-full max-w-sm border border-slate-800 shadow-2xl text-center relative z-[10000]">
          ...
          <p className="text-slate-500 text-xs mb-6 uppercase font-bold tracking-widest">Enter Admin (6-digit) or Manager (4-digit) PIN</p>
          <div className="flex justify-center gap-2 mb-6">{[0, 1, 2, 3, 4, 5].map((i) => (<div key={i} className={`w-2.5 h-2.5 rounded-full transition-all duration-200 ${pin.length > i ? "bg-red-500 scale-125 shadow-lg shadow-red-500/50" : "bg-slate-800"}`} />))}</div>
          ...
          <button onClick={() => setShowForgotPin(true)} className="mt-4 text-slate-500 hover:text-red-400 text-xs font-bold uppercase tracking-widest transition-all">
            Forgot PIN?
          </button>
        </div>
        <ForgotPinModal isOpen={showForgotPin} onClose={() => setShowForgotPin(false)} shopId={staffShopId} />
      </div>
    );
  }
